...
.EXAMPLE
    ...
    .\Register-EntraSoftwarePasskey.ps1 -UserUpn "user@contoso.com" -DisplayName "Test Key" `
...
if (-not $UseManagedIdentity) {
    if (-not $ClientId) {
        throw "ClientId is required. Provide via -ClientId parameter or Initialize-EntraPasskeyKeyVault pipeline."
    }
...
Write-Host "  1. Test authentication with Invoke-EntraPasskeyLogin.ps1" -ForegroundColor White
...
if ($PassThru) {
    # Convert SecureString to plain text for pipeline output (needed by Invoke-EntraPasskeyLogin.ps1)
