...
.EXAMPLE
    .\Invoke-EntraPasskeyLogin.ps1 -KeyFilePath .\passkey.json
...
.EXAMPLE
    ...
    .\Invoke-EntraPasskeyLogin.ps1 -UserPrincipalName user@domain.com -UserHandle "base64handle" -CredentialId "base64id" -PrivateKey $privateKey
...
.EXAMPLE
    ...
    .\Invoke-EntraPasskeyLogin.ps1 -KeyFilePath .\passkey.json -KeyVaultAccessToken $token

    Authenticate with a pre-obtained Key Vault access token.

.EXAMPLE
    .\Invoke-EntraPasskeyLogin.ps1 -KeyFilePath .\passkey.json -KeyVaultClientId "clientid" -KeyVaultClientSecret $secret -KeyVaultTenantId "tenantid"

    Authenticate using a service principal for Key Vault access.

.EXAMPLE
    $auth = .\Invoke-EntraPasskeyLogin.ps1 -KeyFilePath .\passkey.json -PassThru
    if ($auth.Success) {
...
if (-not $KeyFilePath -and (-not $UserPrincipalName -or -not $UserHandle -or -not $CredentialId)) {
        throw "Either provide -KeyFilePath or specify manual parameters (-UserPrincipalName, -UserHandle, -CredentialId, and either -PrivateKey or -KeyVaultName). Or pipe from Register-EntraSoftwarePasskey.ps1 with -PassThru."
}
