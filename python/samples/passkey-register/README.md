...
3. Run the sample:
   ...
   ```powershell
   python .\register_keyvault_passkey.py --user-principal-name user@tenant.onmicrosoft.com tap --tap replace-with-tap
   ```

ESTSAUTH mode:

```powershell
python .\register_keyvault_passkey.py --user-principal-name user@tenant.onmicrosoft.com estsauth --ests-auth replace-with-cookie
```
