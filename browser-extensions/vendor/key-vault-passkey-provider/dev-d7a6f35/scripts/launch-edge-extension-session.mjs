...
async function main() {
  ...
  const extensionStagingRoot = await fs.mkdtemp(path.join(os.tmpdir(), "kvpp-edge-extension-"));
  ...
  try {
    ...
    if (!shimActive) {
      throw new Error(`WebAuthn interception did not activate for ${stagedExtension.permission}. Refusing to continue with native Windows Hello.`);
    }

    console.log(`EDGE_PATH=${edgePath}`);
    console.log(`EXTENSION_DIR=${stagedExtension.extensionDir}`);
    console.log(`TARGET_PERMISSION=${stagedExtension.permission}`);
    ...
    console.log("WEBAUTHN_SHIM=override");
