...
const targetUrl = "https://ctap.dev/login";
...
async function main() {
  if (process.env.KVPP_ALLOW_LIVE_KEY_VAULT_TESTS !== "1") {
    throw new Error("This test creates live Key Vault keys and credential metadata. Set KVPP_ALLOW_LIVE_KEY_VAULT_TESTS=1 to acknowledge those mutations.");
  }
  ...
  const extensionStagingRoot = await fs.mkdtemp(path.join(os.tmpdir(), "kvpp-ctap-extension-"));
  const stagedExtension = await stageTargetScopedExtension(distDir, targetUrl, extensionStagingRoot);
  logStep(`using source profile ${sourceProfile}`);
  logStep(`using automation profile ${automationProfile}`);

  let context;
  try {
    context = await chromium.launchPersistentContext(automationProfile, {
      executablePath: edgePath,
      headless: false,
      ignoreDefaultArgs: ["--disable-extensions"],
      args: [
        `--disable-extensions-except=${stagedExtension.extensionDir}`,
        `--load-extension=${stagedExtension.extensionDir}`
      ]
    });
    ...
    if (shimState.includes("[native code]")) {
      throw new Error(`WebAuthn interception did not activate for ${stagedExtension.permission}. Refusing to invoke native Windows Hello.`);
    }
    logStep("shim active on ctap.dev");

    const createResults = [];
    for (const scenario of createScenarios) {
      const authorizationDialogPromise = completeCeremonyAuthorizationDialog(context);
      const createResult = await runCreate(page, popup, scenario);
      await authorizationDialogPromise;
    ...
    await assertRequiredUvRejected(page);
    logStep("required user verification failed closed as expected");
    ...
    const authorizationDialogPromise = completeCeremonyAuthorizationDialog(context);
    const getPromise = runGet(page, null, "preferred");
    ...
    await fs.rm(extensionStagingRoot, { recursive: true, force: true }).catch(() => {});
  }
}
...
async function handleMicrosoftAuthPages(context) {
  for (const page of context.pages()) {
    ...
    try {
      const emailButton = page.getByText("nathan@sharemylabs.com", { exact: true });
...
async function assertRequiredUvRejected(page) {
  const result = await page.evaluate(async () => {
    try {
      await navigator.credentials.create({
        publicKey: {
          rp: { id: "ctap.dev", name: "ctap.dev" },
          user: {
            id: crypto.getRandomValues(new Uint8Array(16)),
            name: `kvpp-required-uv-${Date.now()}@ctap.dev`,
            displayName: "KVPP Required UV Test"
          },
  ...
  if (!result.rejected || result.name !== "NotSupportedError" || !result.message.includes("cannot truthfully satisfy")) {
    throw new Error(`Required UV did not fail closed as expected: ${JSON.stringify(result)}`);
  }
}
...
async function runGet(page, rawIdBase64Url, userVerification = "preferred") {
  try {
    return await page.evaluate(async ({ encodedRawId, nextUserVerification }) => {
      ...
      const challenge = crypto.getRandomValues(new Uint8Array(32));
      const allowCredentials = encodedRawId
        ? [{
            type: "public-key",
            id: decodeBase64Url(encodedRawId)
          }]
        : undefined;
      ...
      return {
        ...
        mode: encodedRawId ? "allowCredentials" : "discoverable"
      };
    }, {
    ...
    if (!rawIdBase64Url || !message.includes("No stored passkey matched this request.")) {
      throw error;
    }
...
async function completeCeremonyAuthorizationDialog(context, timeoutMs = 20000) {
  const dialog = await waitForCeremonyAuthorizationDialog(context, timeoutMs);
  ...
  while (Date.now() < deadline) {
    ...
    try {
      ...
      throw new Error("The passkey authorization dialog closed unexpectedly.");
    }
  }

  throw new Error("Timed out waiting for the passkey authorization dialog to complete.");
}

async function waitForCeremonyAuthorizationDialog(context, timeoutMs) {
  const deadline = Date.now() + timeoutMs;
  ...
  throw new Error("Timed out waiting for the passkey authorization dialog to open.");
}
