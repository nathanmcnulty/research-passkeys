...
import { resetPinAttemptLimit, verifyPinWithAttemptLimit } from "./pin-attempt-limiter";
...
export async function setPinUv(newPin: string, currentPin?: string, context: PinUvContext = {}): Promise<PinUvStatus> {
  ...
  if (existing) {
    if (!currentPin || !(await verifyPinWithAttemptLimit(() => verifyPinAgainstState(existing, currentPin)))) {
...
export async function removePinUv(currentPin: string, context: PinUvContext = {}): Promise<PinUvStatus> {
  ...
  if (!(await verifyPinWithAttemptLimit(() => verifyPinAgainstState(existing, currentPin)))) {
    throw new DOMException("The current PIN is incorrect.", "NotAllowedError");
  }
...
export async function verifyPinUv(pin: string, context: PinUvContext = {}): Promise<boolean> {
  return verifyPinWithAttemptLimit(async () => {
...
async function getFromLocalStorage<TValue>(key: string): Promise<TValue | undefined> {
  return new Promise((resolve, reject) => {
    chrome.storage.local.get([key], (items: Record<string, unknown>) => {
      ...
      if (runtimeError) {
        reject(new Error(`Unable to read PIN state: ${runtimeError.message}`));
        return;
      }
...
async function setInLocalStorage(key: string, value: unknown): Promise<void> {
  await new Promise<void>((resolve, reject) => {
    chrome.storage.local.set({ [key]: value }, () => {
      const runtimeError = chrome.runtime.lastError;
      if (runtimeError) {
        reject(new Error(`Unable to persist PIN state: ${runtimeError.message}`));
        return;
      }
...
async function removeFromLocalStorage(key: string): Promise<void> {
  await new Promise<void>((resolve, reject) => {
    chrome.storage.local.remove(key, () => {
      const runtimeError = chrome.runtime.lastError;
      if (runtimeError) {
        reject(new Error(`Unable to remove PIN state: ${runtimeError.message}`));
        return;
      }
