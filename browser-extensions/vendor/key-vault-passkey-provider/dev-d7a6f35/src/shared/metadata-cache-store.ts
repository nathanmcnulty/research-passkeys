...
export async function replaceEnvelopesCache(envelopes: BrowserCredentialEnvelope[]): Promise<void> {
  const database = await openDatabase();
  await runTransaction(database, "readwrite", (store) => {
    store.clear();
    for (const envelope of envelopes) {
      store.put({ recordId: envelope.recordId, envelope } satisfies EnvelopeRecord);
    }
  });
}
