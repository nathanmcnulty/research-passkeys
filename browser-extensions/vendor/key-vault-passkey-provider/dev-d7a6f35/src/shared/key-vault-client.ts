...
import { assertKeyVaultRequestUrl, getKeyVaultTokenScope, normalizeKeyVaultBaseUrl, normalizeKeyVaultKeyPath } from "./security-boundaries";
...
export class KeyVaultClient {
  private readonly vaultBaseUrl: string;

  public constructor(
    private readonly config: KeyVaultClientConfig,
    private readonly tokenProvider: AccessTokenProvider
  ) {
    this.vaultBaseUrl = normalizeKeyVaultBaseUrl(config.baseUrl);
  }
  ...
  public async createEcP256Key(keyName: string, keyType: "EC" | "EC-HSM"): Promise<KeyVaultEcKey> {
    const response = await this.sendJson<{ key: { kid: string; x: string; y: string } }>(
      "POST",
      `${normalizeKeyVaultKeyPath(this.vaultBaseUrl, keyName)}/create?api-version=${apiVersion}`,
      {
        kty: keyType,
        crv: "P-256",
        key_ops: ["sign", "verify"],
  ...
  public async getMetadataKeyIdentifier(): Promise<string> {
    const response = await this.sendJson<{ key: { kid: string } }>(
      "GET",
      `${normalizeKeyVaultKeyPath(this.vaultBaseUrl, this.config.metadataWrappingKeyName)}?api-version=${apiVersion}`
    );
  ...
  private async send(
    ...
    assertKeyVaultRequestUrl(url, this.vaultBaseUrl);
  ...
  private async acquireVaultToken(): Promise<AccessToken> {
    try {
      return await this.tokenProvider([getKeyVaultTokenScope(this.vaultBaseUrl)]);
    } catch (error) {
  ...
  private normalizeKeyPath(keyIdentifier: string): string {
    return normalizeKeyVaultKeyPath(this.vaultBaseUrl, keyIdentifier);
  }
