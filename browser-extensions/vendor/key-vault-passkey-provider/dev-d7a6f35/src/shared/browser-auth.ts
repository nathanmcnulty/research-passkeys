...
import { assertGuid, assertOAuthRedirect, getKeyVaultTokenScope, normalizeEntraAuthorityHost } from "./security-boundaries";
...
class InteractiveSignInRequiredError extends Error {
  public constructor(message = "Interactive sign-in is required before the extension can call Key Vault.") {
    super(message);
...
export class BrowserEntraTokenBroker {
  ...
  public getStatus(config: BrowserExtensionConfig): BrowserAuthStatus {
    if (config.metadataTransportMode === "LocalCacheOnly") {
      return {
        mode: "local-only",
        expiresOn: null,
        message: "Local cache mode does not require a Key Vault token."
      };
    }
    ...
    if (this.cachedToken && !isExpired(this.cachedToken.expiresOn)) {
      return {
        ...
        message: "Key Vault token is cached in the extension background session."
      };
    }
    ...
    return {
      mode: "signed-out",
      expiresOn: null,
      message: this.lastError ?? "Interactive sign-in is required before the extension can call Key Vault."
    };
  }
  ...
  public async beginInteractiveSignIn(config: BrowserExtensionConfig): Promise<BrowserAuthStatus> {
    ...
    await this.ensureHydrated();
    const keyVaultScope = getKeyVaultTokenScope(config.keyVaultBaseUrl);

    try {
      const token = await this.runAuthorizationCodeFlow(config, [keyVaultScope], {
        interactive: true,
        prompt: "select_account"
      });
      await this.storeCachedToken(normalizeScopes([keyVaultScope]), token);
      return this.getStatus(config);
    } catch (error) {
  ...
  public async beginSilentSignIn(config: BrowserExtensionConfig): Promise<BrowserAuthStatus | null> {
    const keyVaultScope = getKeyVaultTokenScope(config.keyVaultBaseUrl);
    const token = await this.acquireTokenSilently(config, [keyVaultScope]);
  ...
  public async acquireToken(config: BrowserExtensionConfig, scopes: string[]): Promise<AccessToken> {
    ...
    throw new InteractiveSignInRequiredError(this.lastError ?? "Interactive sign-in is required before the extension can call Key Vault.");
  }
  ...
  public async acquireTokenSilently(config: BrowserExtensionConfig, scopes: string[]): Promise<AccessToken | null> {
    ...
    try {
      ...
      if (isInteractiveSignInRequired(error)) {
        ...
        await this.storeLastError("Interactive sign-in is required before the extension can call Key Vault.");
        return null;
      }
  ...
  private async runAuthorizationCodeFlow(
    ...
    const redirectUrl = assertOAuthRedirect(redirectResponse, redirectUri, state);
...
export function buildAuthorityBase(config: BrowserExtensionConfig): string {
  const authorityHost = normalizeEntraAuthorityHost(config.authorityHost);
  const tenantId = assertGuid(config.tenantId, "Tenant ID");
