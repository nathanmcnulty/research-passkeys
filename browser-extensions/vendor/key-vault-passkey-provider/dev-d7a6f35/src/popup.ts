...
async function initialize() {
  ...
  signOutButton.addEventListener("click", async () => {
    ...
    clearUnlockInput();
    await refreshAll();
    setStatus("Cleared cached Key Vault token state for this extension. Unlock with the extension PIN to continue.");
  });
  ...
  storedCredentialListElement.addEventListener("click", async (event) => {
    ...
    if (target.dataset.action === "toggle-enabled") {
      ...
      if (!enable && !window.confirm(`Disable the stored passkey for ${entry.rpId} (${entry.userDisplayName})?`)) {
        return;
      }

      target.disabled = true;
      const response = await sendRuntimeMessage({ kind: "set-stored-credential-enabled", recordId, enabled: enable });
      if (!response.ok || !("storedCredentials" in response)) {
        target.disabled = false;
        setStatus(response.ok ? "Unable to change the stored passkey." : response.error.message, true);
        return;
      }

      renderStoredCredentials(response.storedCredentials);
      await refreshMetadataSummary();
      setStatus(`${enable ? "Enabled" : "Disabled"} stored passkey for ${entry.rpId}.`);
      return;
    }

    const removalVerb = entry.state === "pending" ? "Clean up the incomplete" : "Remove the stored";
    const confirmed = window.confirm(`${removalVerb} passkey for ${entry.rpId} (${entry.userDisplayName})?`);
    if (!confirmed) {
      return;
    }
    ...
    renderStoredCredentials(response.storedCredentials);
    await refreshMetadataSummary();
    setStatus(`${entry.state === "pending" ? "Cleaned up incomplete" : "Removed stored"} passkey for ${entry.rpId}.`);
  });
...
async function refreshStoredCredentials() {
  ...
  if (state.config.metadataTransportMode === "KeyVaultSecrets" && authStatus.mode !== "signed-in") {
    ...
    storedCredentialListStatusElement.textContent = state.lockedAt
      ...
      : "Sign in to Key Vault to view stored passkeys.";
    return;
  }
...
function renderStoredCredentials(records: StoredCredentialSummary[]) {
  ...
  if (records.length === 0) {
    ...
    if (!storedCredentialListStatusElement.textContent) {
  ...
  for (const [rpId, groupRecords] of groupedRecords.entries()) {
    ...
    for (const record of groupRecords) {
      ...
      if (record.state === "active" || record.state === "disabled") {
        ...
        toggleButton.className = "secondary compact";
        toggleButton.dataset.recordId = record.recordId;
        toggleButton.dataset.action = "toggle-enabled";
        toggleButton.textContent = record.state === "disabled" ? "Enable" : "Disable";
      ...
      removeButton.textContent = record.state === "pending"
        ? "Clean up"
        : record.state === "deleting" ? "Retry removal" : "Remove";
      actions.append(removeButton);

      titleRow.append(credentialTitleBlock, actions);

      const details = document.createElement("p");
      details.className = "credential-item-details";
      const lifecycleDetail = record.state === "active" ? "" : ` State ${record.state}.`;
      details.textContent = `Updated ${record.updatedAt}. Sign count ${record.signCount}.${lifecycleDetail}`;
...
function renderOverview() {
  ...
  setupDetailElement.textContent = locked
    ? interactiveRecoveryActive
      ? "PIN verified. Complete interactive sign-in to restore a fresh Key Vault session."
  ...
  unlockHintElement.textContent = interactiveRecoveryActive
    ...
    : "Enter the extension PIN to unlock. If browser SSO is still active, the session will recover silently.";
}
...
function formatLockReason(reason: ExtensionState["lockReason"]): string {
  ...
  if (reason === "token-expired") {
    return " A fresh sign-in may be needed if silent SSO cannot recover the session.";
  }
...
async function hasCurrentSiteAccess(originPattern: string): Promise<boolean> {
  return new Promise((resolve) => {
