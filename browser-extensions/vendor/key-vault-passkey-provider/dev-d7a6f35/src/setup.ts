...
const authorityHostInput = requireElement<HTMLInputElement>("#authorityHost");
const keyVaultBaseUrlInput = requireElement<HTMLInputElement>("#keyVaultBaseUrl");
const signingKeyNameInput = requireElement<HTMLInputElement>("#signingKeyName");
const signingKeyTypeInput = requireElement<HTMLSelectElement>("#signingKeyType");
const metadataWrappingKeyNameInput = requireElement<HTMLInputElement>("#metadataWrappingKeyName");
const metadataTransportModeInput = requireElement<HTMLSelectElement>("#metadataTransportMode");
const metadataSecretPrefixInput = requireElement<HTMLInputElement>("#metadataSecretPrefix");
...
const lockTimeoutOptions = Array.from({ length: 56 }, (_value, index) => index + 5);
...
function bindEvents() {
  ...
  saveConfigButton.addEventListener("click", async () => {
    const response = await sendRuntimeMessage({ kind: "save-config", config: readConfig(), lockTimeoutMinutes: readLockTimeoutMinutes() });
  ...
  signOutButton.addEventListener("click", async () => {
    ...
    await refreshAll();
    navigateToStep("auth");
    setStatus("Cleared cached Key Vault token state.");
  });
...
function renderConfig(state: ExtensionState) {
  ...
  authorityHostInput.value = config.authorityHost;
  keyVaultBaseUrlInput.value = config.keyVaultBaseUrl;
  signingKeyNameInput.value = config.signingKeyName;
  signingKeyTypeInput.value = config.signingKeyType;
  metadataWrappingKeyNameInput.value = config.metadataWrappingKeyName;
  metadataTransportModeInput.value = config.metadataTransportMode;
  metadataSecretPrefixInput.value = config.metadataSecretPrefix;
  lockTimeoutMinutesInput.value = clampLockTimeoutSelection(state.lockTimeoutMinutes).toString();
}
...
function renderAuth(config: BrowserExtensionConfig, authStatus: BrowserAuthStatus) {
  ...
  authStepHintElement.textContent = localOnly
    ? "Local cache mode does not require a Key Vault sign-in. Unlock still requires the extension PIN after idle timeout or browser restart."
    : "Interactive sign-in is required before the PIN can be protected with Key Vault wrapping. Later unlocks stay PIN-first and try silent SSO before interactive sign-in.";
...
function renderPin(config: BrowserExtensionConfig, pinStatus: PinUvStatus, authStatus: BrowserAuthStatus) {
  ...
  const requiresSignedInKeyVault = config.metadataTransportMode === "KeyVaultSecrets" && authStatus.mode !== "signed-in";
  ...
  pinHintElement.textContent = pinStatus.isConfigured
    ...
    : requiresSignedInKeyVault
      ? "Complete Key Vault sign-in first so the initial PIN verifier can be wrapped by Key Vault while still remaining locally verifiable for unlock."
...
function readConfig(): BrowserExtensionConfig {
  return {
    ...
    authorityHost: authorityHostInput.value.trim() || "https://login.microsoftonline.com",
    keyVaultBaseUrl: keyVaultBaseUrlInput.value.trim(),
    signingKeyName: signingKeyNameInput.value.trim() || "passkey-signing-key",
    signingKeyType: signingKeyTypeInput.value === "EC-HSM" ? "EC-HSM" : "EC",
    metadataWrappingKeyName: metadataWrappingKeyNameInput.value.trim() || "passkey-metadata-wrap-key",
    metadataTransportMode: metadataTransportModeInput.value === "LocalCacheOnly" ? "LocalCacheOnly" : "KeyVaultSecrets",
    metadataSecretPrefix: metadataSecretPrefixInput.value.trim() || "kvpp-md"
  };
}

function readLockTimeoutMinutes(): number {
  return clampLockTimeoutSelection(Number(lockTimeoutMinutesInput.value));
}
...
function populateLockTimeoutOptions() {
  ...
  for (const timeoutMinutes of lockTimeoutOptions) {
    const option = document.createElement("option");
    option.value = timeoutMinutes.toString();
    option.textContent = `${timeoutMinutes} minute${timeoutMinutes === 1 ? "" : "s"}`;
    lockTimeoutMinutesInput.append(option);
  }
}

function clampLockTimeoutSelection(value: number): number {
