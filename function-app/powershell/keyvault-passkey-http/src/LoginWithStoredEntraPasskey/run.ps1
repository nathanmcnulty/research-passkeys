...
try {
    ...
    Push-OutputBinding -Name Response -Value (New-JsonHttpResponse -StatusCode OK -Body @{success=[bool]$login.Result.Success;provider='entra';recordId=$recordId;userPrincipalName=$login.Result.UserPrincipalName;estsAuth=$login.ESTSAuthCookie})
