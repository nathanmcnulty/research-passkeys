...
.EXAMPLE
    ...
    .\Register-EntraKeyVaultPasskeyViaEstsAuth.ps1 -ESTSAuthCookie "1.AVIA..." -KeyVaultName "my-keyvault"
...
if ($kvKeyNameIsAutoGenerated) {
    $upnPrefix = $UserPrincipalName.Split('@')[0] -replace '[^0-9A-Za-z-]', ''
    if ([string]::IsNullOrWhiteSpace($upnPrefix)) { $upnPrefix = 'user' }
    $prefix = if ($upnPrefix.Length -gt 12) { $upnPrefix.Substring(0, 12) } else { $upnPrefix }
    $randomSuffix = Get-Random -Minimum 100000 -Maximum 1000000
    $kvKeyNameResolved = "pk-$prefix-$randomSuffix"
}

if ([string]::IsNullOrWhiteSpace($PasskeyDisplayName)) {
    $PasskeyDisplayName = "pk$(Get-Random -Minimum 1000 -Maximum 10000)"
}
