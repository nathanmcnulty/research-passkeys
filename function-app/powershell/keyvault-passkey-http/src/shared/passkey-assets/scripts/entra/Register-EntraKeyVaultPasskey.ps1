...
.DESCRIPTION
    ...
    Invoke-EntraPasskeyLogin.ps1 for subsequent authentication.
...
.EXAMPLE
    .\Register-EntraKeyVaultPasskeyViaTap.ps1 -TAP "abc123" -UserPrincipalName "user@contoso.com" `
        -TenantId "847b5907-..." -KeyVaultName "my-keyvault"

.EXAMPLE
    .\Register-EntraKeyVaultPasskeyViaTap.ps1 -TAP $tap -UserPrincipalName "user@contoso.com" `
...
Import-Module (Join-Path -Path $moduleRoot -ChildPath 'Passkey.EntraAuth\Passkey.EntraAuth.psm1') -Force
...
if (-not $KeyVaultKeyName) {
    $sanitizedUpnPrefix = ($UserPrincipalName.Split('@')[0] -replace '[^0-9A-Za-z-]', '')
    if ([string]::IsNullOrWhiteSpace($sanitizedUpnPrefix)) {
...
Write-Host ""
$authCode = Invoke-PasskeyTapPkceLogin `
    -Authority $TenantId `
    -ClientId $ClientId `
...
Add-PasskeyBrowserHeaders -Headers $newHeaders
...
try {
    ...
    $newErrResp = $_.Exception.Response
    $newErrBody = if ($_.ErrorDetails.Message) { $_.ErrorDetails.Message } else { $null }
    if (-not $newErrBody -and $newErrResp) {
        try {
            $newErrStream = $newErrResp.GetResponseStream()
            $newErrReader = [System.IO.StreamReader]::new($newErrStream)
            $newErrBody = $newErrReader.ReadToEnd()
            $newErrReader.Close()
        } catch {
        }
    }

    $newErrHeaders = [ordered]@{}
    if ($newErrResp) {
        foreach ($headerName in @('x-ms-request-id', 'x-ms-ests-server', 'WWW-Authenticate', 'x-ms-diagnostics', 'Retry-After', 'x-ms-srs')) {
            try {
                $headerValue = $newErrResp.Headers[$headerName]
                if (-not [string]::IsNullOrWhiteSpace([string]$headerValue)) {
                    $newErrHeaders[$headerName] = [string]$headerValue
                }
            } catch {
            }
        }
    ...
    $newErrStatusCode = if ($null -eq $newErrResp) { 'unknown' } else { [int]$newErrResp.StatusCode }
    $newErrHeaderSnippet = if ($newErrHeaders.Count -eq 0) { '(none)' } else { ($newErrHeaders | ConvertTo-Json -Compress) }
    $newErrBodySnippet = if ([string]::IsNullOrWhiteSpace($newErrBody)) { '(empty)' } else { $newErrBody.Substring(0, [Math]::Min(1000, $newErrBody.Length)) }
    throw "authenticationmethods/new failed: HTTP $newErrStatusCode. ResponseHeaders=$newErrHeaderSnippet; ResponseBody=$newErrBodySnippet"
}
...
Add-PasskeyBrowserHeaders -Headers $verifyHeaders
...
if (-not $OutputPath) {
    ...
    $OutputPath = Join-Path (Get-Location) "${sanitizedUpnPrefix}_passkey_$timestamp.json"
}
...
Write-Host "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━" -ForegroundColor Green
Write-Host ""
Write-Host "  Use Invoke-EntraPasskeyLogin.ps1 with the credential file to authenticate:" -ForegroundColor Gray
Write-Host "    .\Invoke-EntraPasskeyLogin.ps1 -KeyFilePath ""$OutputPath"" ``" -ForegroundColor Gray
